    ...
    def loss(self,
        ...
        if num_pos > 0:
            ...
            if self.use_vfl:
                ...
                neg_inds = torch.where(
                    ((flatten_labels < 0) & (flatten_labels < bg_class_ind)) <= 0)[0]
                neg_decoded_bbox_preds = distance2bbox(flatten_points[neg_inds], flatten_bbox_preds_refine[neg_inds])
                neg_decoded_target_preds = distance2bbox(flatten_points[neg_inds], flatten_bbox_targets[neg_inds])
                iou_targets_raf_2 = bbox_overlaps(
                    neg_decoded_bbox_preds,
                    neg_decoded_target_preds.detach(),
                    is_aligned=True)
                neg_ious = iou_targets_raf_2.clone().detach()
                a = (neg_ious == 0).sum()
